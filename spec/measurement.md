...
- **Embeddings come from a model that is *separate* from the generation model.** We use a Llama-3 embedding model via Together AI for this. The reason: if the embedding model and the generation model are the same, the embeddings may overfit to the generator's idiosyncrasies and underestimate semantic spread. A separate, neutral embedder gives a more honest measure of how distinct the responses really are.
...
| Embedding model identity (Llama-3 via Together AI) | **No, version bump required** | A different embedder produces different cluster geometry, which changes every H value. |
